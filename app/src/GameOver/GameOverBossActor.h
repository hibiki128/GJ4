...
/// ただし踏み替えの敷居（stepTrigger）はゲーム中の値のままだと歩き回る前提の広さで、
/// その場の小さな揺れでは一生届かず脚が止まって見える。ここでは歩かないので、
/// 敷居を揺れ幅に合わせて下げてから使う。
///
...
private:
    ...
    /// <summary>
    /// 踏み替えの敷居を、いまの揺れ幅に合わせて入れ直す。
    /// ゲーム中の値（歩いて移動する前提の広さ）のままだと、その場の揺れでは届かない
    /// </summary>
    void ApplyStepTrigger();
    ...
    // 踏み替えの敷居を揺れ幅の何割にするか。小さいほど細かく踏み替える。
    // 1.0 を超えると揺れきっても届かなくなり、脚が止まる
    float stepTriggerRatio_ = 0.45f;
